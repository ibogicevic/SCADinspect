...
public class SettingsDialog {
  ...
  public static void openDialog() {
    ...
    if (result.orElse(ButtonType.CANCEL) == okButtonType) {
      boolean staticAnalysisOld = userPrefs
          .getBoolean(SettingsDialog.SETTING_STATIC_ANALYSIS, false);
      ...
      if (documentation.isSelected() != documentationOld) {
        if (documentation.isSelected()) {
          ...
          Main.getInstance().tabArea.getTabs()
              .remove(Main.getInstance().tabArea.getDocumentation());
        }
